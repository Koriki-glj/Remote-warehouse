	...
	do i=1,nxt(m)
		do j=1,nyt(m)
			do k=1,nzt(m)
				...
				zlast(i,j,k,m)=lz*yt(1,k)  !为何这里第三维拉伸用yt(1,k)的数据？,因为yt(1,k)一直到yt(481,k)的数据都是一样的
	...
	!--------------------------------------------------------------------
	write(*,*)'区块10外圆边界靠近y轴第一个点（最后一个点）的x值temporary(15)是：',temporary(15)
	write(*,*)'区块10外圆边界靠近y轴第一个点（最后一个点）的y值temporary(16)是：',temporary(16)
	...
	!--------------------------------------------------------------------
	do j=1,2*n-1
		xss(1,j)= (dm/2.0+temp1)*cos(pi/4.0)+dm/9.0*cos(pi/4.0)*sb(2,j)
		yss(1,j)=-(dm/2.0+temp1)*sin(pi/4.0)+dm/9.0*sin(pi/4.0)*sb(2,j)
	...
	!--------------------------------------------------------------------
	do j=1,2*n-1
		xss(1,j)=temporary(17)+(7.0*dm/54.0+zd2/3.0)*cos(pi/4.0)*sb(2,j)
		yss(1,j)=temporary(18)+(7.0*dm/54.0+zd2/3.0)*sin(pi/4.0)*sb(2,j)
	...
	!--------------------------------------------------------------------
	do j=1,2*n-1
		xss(1,j)=temporary(19)+(7.0*dm/54.0+zd2/3.0)*cos(pi/4.0)*sb(2,j)
		yss(1,j)=temporary(20)+(7.0*dm/54.0+zd2/3.0)*sin(pi/4.0)*sb(2,j)
	...
	write(8,*) nblocks
	write(8,*)(imax(m),jmax(m),kmax(m),m=1,29)
	do m=1,29
		write(8,*) &
			&    (((xlast(i,j,k,m),i=1,imax(m)),j=1,jmax(m)),k=1,kmax(m)),&
			&    (((ylast(i,j,k,m),i=1,imax(m)),j=1,jmax(m)),k=1,kmax(m)),&
