	...
	!********************************声明*******************************
	!边界和内部坐标
	real(kind=8):: xs(2,2,200),ys(2,2,200)
	...
	!-----------------金字塔————--------------------------------------
	!内部坐标
	real(kind=8):: yt(501,501),zt(501,501),xt(501,501)
	! 边界坐标
	real(kind=8):: xss(2,501),yss(2,501)
	!节点数
	...
	!尺寸
	real(kind=8):: ra,rb,rh,alfa
	!临时一维数组
	real(kind=8):: temporary(20)
	!临时二维数组
	real(kind=8):: temporary1(8,501)
	!------------强化管外形--------------------------------------------------------
	real(kind=8):: ztnew1(501,501),xtnew1(501,501),ytnew1(501,501)
	real(kind=8):: rt(501,201),thetat(501,201)
	real(kind=8):: de,xL,da,yL
	!------------外置圆形---------------------------
	real(kind=8):: ztnew2(501,301),xtnew2(501,301),ytnew2(501,301)
	real(kind=8):: dm
	!********************************尺寸*******************************
	xL=4.47046286
	yL=1.10993419
	...
	!--------------------------------------------------------------------
	do i=1,2*n-1
		xss(1,i)=0.5*da-0.25*da*sb(1,i)        !da是等腰三角形的腰长
		yss(1,i)=0.25*da                  
		xss(2,i)=xss(1,i)                 
		yss(2,i)=0.5*da                       
	end do
	...
	do j=1,2*n-1
		...
		xss(2,j)=0.25*da
		yss(2,j)=0.25*da+0.25*da*sb(2,j)
	end do
	!--------------------------------------------------------------------
		n=int(nxt(m)/2)+1
	p=1.0
	...
	do i=1,2*n-1
		...
		xss(2,i)=0.25*da+0.25*da*sb(1,i)  
		yss(2,i)=0.25*da
	end do
	!--------------------------------------------------------------------
	n=int(nyt(m)/2)+1
	...
	temporary(1)=xss(2,1)
	temporary(2)=yss(2,1)
	write(*,*)'区块4金字塔边界靠近x轴第一个点的x值temporary(1)是：',temporary(1)
	write(*,*)'区块4金字塔边界靠近x轴第一个点的y值temporary(2)是：',temporary(2)
	pause
	!--------------输出区块4网格------------------------------------
	open(2,file='区块4-3d.plt')
	write(2,*) 'variables= "x","y","z"'
	write(2,*) 'zone i=',nzt(m),'j=',nyt(m),'k=',nxt(m)
	do i=1,nxt(m)
		do j=1,nyt(m)
	...
	write(*,*)'区块6外圆边界的x值temporary1(1,j)是：',(temporary1(1,j),j=1,nyt(m))
	write(*,*)'区块6外圆边界的y值temporary1(2,j)是：',(temporary1(2,j),j=1,nyt(m))
	pause
	!--------------------------------------------------------------------
	...
	write(*,*)'区块7金字塔边界靠近y轴的第一个点x值temporary(3)是：',temporary(3)
	write(*,*)'区块7金字塔边界靠近y轴的第一个点y值temporary(4)是：',temporary(4)
	pause
	!--------------------------------------------------------------------
	...
	do j=1,2*n-1
		...
		xs(2,2,j)= (dm/2.0)*dcos(pi/4.0)+(dm/2.0+zd2)*dcos(pi/4.0)+(dm/2.0)*dcos(0.75*pi)*sb(2,j)
		ys(2,2,j)=-(dm/2.0)*dsin(pi/4.0)+(dm/2.0+zd2)*dsin(pi/4.0)+(dm/2.0)*dsin(0.75*pi)*sb(2,j)
	end do
	!--------------------------------------------------------------------
	n=int(nxt(m)/2)+1
	p=1.0
	...
	do j=1,2*n-1
		...
		xs(2,2,j)=(dm/2.0+zd2)*dcos(pi/4.0)+(dm/2.0)*dcos(0.75*pi)*sb(2,j)
		ys(2,2,j)=(dm/2.0+zd2)*dsin(pi/4.0)+(dm/2.0)*dsin(0.75*pi)*sb(2,j)
	end do
	!--------------------------------------------------------------------
	n=int(nxt(m)/2)+1
	p=1.0
	...
	!--------------------------------------------------------------------
	do j=1,2*n-1
		xss(1,j)= dm/2.0*dcos(pi/4.0)+temp1*dcos(pi/4.0)*sb(2,j)
		yss(1,j)=-dm/2.0*dsin(pi/4.0)-temp1*dsin(pi/4.0)*sb(2,j)
	...
	!--------------------------------------------------------------------
	do i=1,2*n-1
		xss(1,i)=temporary(3)-temp3*dcos(pi/4.0)*sb(1,i)
		yss(1,i)=temporary(4)-temp3*dsin(pi/4.0)*sb(1,i)
	...
	do i=1,nxt(m)
		...
		xss(2,i)=temporary(3)-temp3*dcos(pi/4.0)*sb(1,i)
		yss(2,i)=temporary(4)-temp3*dsin(pi/4.0)*sb(1,i)
	end do
	!--------------------------------------------------------------------
	n=nyt(m)
	p=1.4
...
do i=1,nxt(m)
	...
	!--------------------------------------------------------------------
	do i=1,nxt(m)
		xss(2,i)=temporary(1)-temp3*dcos(pi/4.0)*sb(1,i)
		yss(2,i)=temporary(2)-temp3*dsin(pi/4.0)*sb(1,i)
		xss(1,i)= dsin(pi/4.0)*da*dcos(pi/4.0)-temp3*dcos(pi/4.0)*sb(1,i)
		yss(1,i)=-dsin(pi/4.0)*da*dsin(pi/4.0)-temp3*dsin(pi/4.0)*sb(1,i)
	end do
	!--------------------------------------------------------------------
	...
	write(8,*) nblocks
	write(8,*)(imax(m),jmax(m),kmax(m),m=4,16)
	do m=4,16
		write(8,*) &
			&    (((xlast(i,j,k,m),i=1,imax(m)),j=1,jmax(m)),k=1,kmax(m)),&
			&    (((ylast(i,j,k,m),i=1,imax(m)),j=1,jmax(m)),k=1,kmax(m)),&
