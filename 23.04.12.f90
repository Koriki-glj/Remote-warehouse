	...
	!write(*,*) 'xt(17,21)=',xt(17,21)
	!pause
	!
	!write(*,*) 'xt(17,41)=',xt(17,41)
	!pause
	!--------------------------------------------------------------------
	!上述程序将整体金字塔板模型复制到一个新位置，其起点坐标为(nra-1)*ra，(nrz-1)*rb；
	...
	do i=1,nxtemp			 !扩展后的金字塔板长=nra*ra=xL,宽=nrz*rb
		do  j=1,nytemp
			do   k=1,1
				write(2,*)xt(i,j),yt(i,j),zt(i,j)
			end do
	...
	close(2)
	write(*,*)'扩展后金字塔板-yt(1,k)',(yt(1,k),k=1,nytemp)
	pause
