	...
	do j=1,nytemp
	do k=1,nztemp
		zjl(j,k)=lz/(nztemp-1)*(k-1)
	end do
	end do
	write(*,*)'zjl(1,k)=',(zjl(1,k),k=1,nztemp)
	pause
	
	write(*,*)'zjl(5,k)=',(zjl(5,k),k=1,nztemp)
	pause
	...
	close(2)
	write(*,*)'金字塔型强化管外形-ztnew1(1,k)',(ztnew1(1,k),k=1,nztemp)
	pause
	!------------外置圆形------------------------------------------------
	do j=1,nxtemp
		do k=1,nztemp
	...
	close(2)
	write(*,*)'外置圆形-ztnew2(1,k)',(ztnew2(1,k),k=1,nztemp)
	pause
	!***************************第一部分*********************************
	!由固体域，流体域组成
	!流体域包括等腰三角形和两个流体小块
